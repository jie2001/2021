...
(function () {
	...
	// 当页面刷新的时候 刷新之前是哪个页面 刷新之后还应该是哪个页面 (第一张用localstorage)去做 第二种用hash值去做 因为他们刷新的时候值不会变化
	let initIndex = 0;
	// 通过hash值去改变initindex值 如果当前页面hash是organize 把initindeex对应的值是0 如果hash的值是customer 那initindex对应的值是1
	...
	change(0); // 让页面初次渲染的时候让change执行一下，传递实参0，代表默认显示第一大组(organize)
