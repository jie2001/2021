let userListModule = (function () {
	...
	let bindSelect = () => {
		return axios.get('/department/list').then((res) => {
			...
			if (code == 0) {
				let str = `<option value="0">全部</option>`;// 下拉框的
				// str默认包含一个全部的option，
				data.forEach(item => {
					let {
						id,
...
// 在这里存放的就是选框的逻辑
	let checkbox=()=>{
		console.log(1)
		let allCheck=$headTh.find('input');
		let everyCheck=$('tbody').find('input')
		console.log(allCheck,everyCheck)
		allCheck.click(function(){
			let flag=$(this).prop('checked');
			// 当前全选框 选中就是true 没有选中那就是false
			everyCheck.prop('checked',flag)
			// 把当前的全选框的状态flag赋值给每一个小框
			console.log(flag)
		})
		everyCheck.click(function(){
			// 循环everyCheck。看看每一个小框的状态 如果有一个是false 那就把全选框改为false 如果是true 那就把全选框改为true
			let flag=true;
			everyCheck.each((index,item)=>{
				let checked=$(item).prop('checked');
				if(!checked){
					flag=false;
					return; // 当前执行的回调函数如果return false就会结束当前的each循环
				}
				// return false;
			});
